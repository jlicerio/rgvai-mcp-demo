"""cli_command tools for calculator-demo."""
...
def register_tools(server: FastMCP, _get_backend) -> None:
    ...
    async def history(
        ...
        # Run CLI subcommand: history
        cmd_args: list[str] = []
        if args:
            cmd_args.extend(args.split())
        return await _get_backend().run_subcommand("history", cmd_args)
    ...
    async def sqrt(
        ...
        """Square root"""
        # Run CLI subcommand: sqrt
        cmd_args: list[str] = []
        if a is not None:
            cmd_args.append(str(a))
        return await _get_backend().run_subcommand("sqrt", cmd_args)
