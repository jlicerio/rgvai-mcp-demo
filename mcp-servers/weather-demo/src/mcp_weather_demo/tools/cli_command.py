"""cli_command tools for weather-demo."""
...
def register_tools(server: FastMCP, _get_backend) -> None:
    ...
    async def compare(
        ...
        # Run CLI subcommand: compare
        cmd_args: list[str] = []
        if city1 is not None:
            cmd_args.append(str(city1))
        if city2 is not None:
            cmd_args.append(str(city2))
        return await _get_backend().run_subcommand("compare", cmd_args)
    ...
    async def current(
        ...
        # Run CLI subcommand: current
        cmd_args: list[str] = []
        if city is not None:
            cmd_args.append(str(city))
        return await _get_backend().run_subcommand("current", cmd_args)
    ...
    async def forecast(
        ...
        # Run CLI subcommand: forecast
        cmd_args: list[str] = []
        if city is not None:
            cmd_args.append(str(city))
        if days is not None:
            cmd_args.extend(["--days", str(days)])
        return await _get_backend().run_subcommand("forecast", cmd_args)
