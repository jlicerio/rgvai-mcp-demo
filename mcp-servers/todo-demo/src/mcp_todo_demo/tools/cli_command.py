"""cli_command tools for todo-demo."""
...
def register_tools(server: FastMCP, _get_backend) -> None:
    ...
    async def add(
        ...
        """Add a task"""
        # Run CLI subcommand: add
        cmd_args: list[str] = []
        if text is not None:
            cmd_args.append(str(text))
        return await _get_backend().run_subcommand("add", cmd_args)
    ...
    async def clear(
        ...
        """Delete all tasks"""
        # Run CLI subcommand: clear
        cmd_args: list[str] = []
        if args:
            cmd_args.extend(args.split())
        return await _get_backend().run_subcommand("clear", cmd_args)
    ...
    async def delete(
        ...
        """Delete a task"""
        # Run CLI subcommand: delete
        cmd_args: list[str] = []
        if id is not None:
            cmd_args.append(str(id))
        return await _get_backend().run_subcommand("delete", cmd_args)
    ...
    async def done(
        ...
        """Mark a task as done"""
        # Run CLI subcommand: done
        cmd_args: list[str] = []
        if id is not None:
            cmd_args.append(str(id))
        return await _get_backend().run_subcommand("done", cmd_args)
    ...
    async def list(
        ...
        """List all tasks"""
        # Run CLI subcommand: list
        cmd_args: list[str] = []
        if args:
            cmd_args.extend(args.split())
        return await _get_backend().run_subcommand("list", cmd_args)
